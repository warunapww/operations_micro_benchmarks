...
//	printf("5\n");
    ...
    double nGFlopsPerSec = 1e3*nFlops/exec_time;
    ...
    double nGBytesPerSec = 1e3*nBytes/exec_time;

	// Report timing data.
    printf( "%d %d %d %d Time: %f (ms), GFLOPS: %f GBytesS: %f\n", 
             GridWidth, BlockWidth, k, ValuesPerThread, time, nGFlopsPerSec, nGBytesPerSec);
